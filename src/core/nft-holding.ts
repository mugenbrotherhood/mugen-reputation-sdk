...
export class NFTHoldingAnalyzer {
   ...
   */
  async getHoldingScore(walletAddress: string): Promise<NFTHoldingResult> {
    try {
  ...
  private getHoldingRecommendation(result: NFTHoldingResult): string {
    if (result.score >= 0.8) return "Excellent long-term commitment to Azuki";
    if (result.score >= 0.6) return "Good holding pattern, consider increasing long-term positions";
    if (result.score >= 0.3) return "Moderate holding, focus on long-term accumulation";
