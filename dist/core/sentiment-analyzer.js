...
class SentimentAnalyzer {
    constructor(config) {
        this.config = config;
        this.twitter = new twitter_api_v2_1.TwitterApi(config.twitter.bearerToken);
    ...
    async analyzeSentiment(username, options = {}) {
        const { tweetCount = 50, threshold = 0.6, includeReplies = true } = options;
        try {
            // 1. Get user profile
            ...
            const eligibilityReasons = this.getEligibilityReasons({ textSentiment, accountAge, engagement, crossPlatform, consistency, authenticity }, threshold);
            return {
            ...
            throw new Error(`Failed to analyze sentiment for ${username}: ${error.message}`);
        }
    }
    async fetchUserTweets(userId, count, includeReplies) {
    ...
    calculateAccountAgeScore(createdAt) {
    ...
    async calculateCrossPlatformScore(userId, username) {
    ...
    calculateConsistencyScore(tweets) {
        if (tweets.length < 3)
            return 0;
    ...
    calculateAuthenticityScore(relevantTweets, allTweets) {
        let score = 1.0;
        ...
        const burstTweets = allTweets.filter(t => new Date(t.created_at).getTime() > recentHour).length;
        if (burstTweets > 10)
            score -= 0.2;
    ...
    getEligibilityReasons(scores, threshold) {
        const reasons = [];
        if (scores.textSentiment < 0.3)
            reasons.push("Low positive sentiment in tweets");
        if (scores.accountAge < 0.2)
            reasons.push("Account too new");
        if (scores.engagement < 0.2)
            reasons.push("Limited community engagement");
        if (scores.authenticity < 0.5)
            reasons.push("Authenticity concerns detected");
        return reasons;
    }
    getStrongestSignal(scores) {
        return Object.entries(scores).reduce((best, [key, value]) => value > scores[best] ? key : best);
    }
    getRecommendation(overallScore, scores) {
        if (overallScore >= 0.8)
            return "Excellent Mugen community member";
        if (overallScore >= 0.6)
            return "Good community engagement, consider increasing activity";
        if (overallScore >= 0.4)
            return "Moderate engagement, focus on authentic Azuki content";
